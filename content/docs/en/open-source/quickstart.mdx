...
- **`@tool`** — expose any async Python function as a tool with automatic schema generation
- **`@task`** — use plain Python functions as durable agent steps
- **Durable execution** — resume from failure with `jamjet dev`
- **Runtime-enforced limits** — iterations, cost, and timeouts as first-class controls
...
## Try crash recovery in 90 seconds
...
3. **Kill the worker during execution** (Ctrl+C mid-run, or kill the process)
...
5. **Watch the workflow resume from the last completed checkpoint**

**Result:** No re-running earlier steps. No wasted API calls. No lost state.
