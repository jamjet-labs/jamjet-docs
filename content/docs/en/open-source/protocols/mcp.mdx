...
> **Key difference:** JamJet treats MCP tool calls as part of a durable workflow, not just ephemeral tool invocations. That means tool use remains checkpointed, replayable, and observable — even if the process crashes mid-call.
...
- [A2A Integration](/open-source/protocols/a2a) — delegate tasks to other agents
- [Showcase](https://jamjet.dev/showcase) — see MCP patterns in production examples
