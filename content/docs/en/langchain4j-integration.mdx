...
### Engram: long-term memory beyond chat history

`JamjetChatMemoryStore` persists raw conversation messages through the JamJet runtime. For agents that need richer memory --- entity extraction, temporal knowledge graphs, semantic search --- consider [Engram](https://java-ai-memory.dev), JamJet's dedicated memory layer. The `engram-spring-boot-starter` provides a `ChatMemoryRepository` for Spring AI that stores messages through the Engram server, which additionally extracts entities and relations and indexes them for semantic recall. See the [Engram Memory section](/spring-boot-starter#engram-memory) in the Spring Boot Starter guide for setup details.
