...
Policy blocks compile to runtime IR from SDK 0.10.1.
...
The following sequence uses the `jamjet` CLI. Replace `exec_01...` with the execution ID printed by `jamjet run`.

**1. Start the workflow.**

```bash
jamjet run workflow.yaml --input '{"payment_id": "pay_001"}'
```

Or register and start via the REST API:

```bash
curl -s -X POST http://localhost:7700/workflows \
  -H "Content-Type: application/json" \
  -d @workflow.json

curl -s -X POST http://localhost:7700/executions \
  -H "Content-Type: application/json" \
...
```bash
jamjet approvals exec_01JM4X8NKWP2
```

Or via REST:

```bash
curl -s http://localhost:7700/executions/exec_01JM4X8NKWP2/approvals
```
...
```bash
jamjet approve exec_01JM4X8NKWP2 --decision approved --comment "Amount verified."
```

Or via REST:

```bash
curl -s -X POST http://localhost:7700/executions/exec_01JM4X8NKWP2/approve \
  -H "Content-Type: application/json" \
  -d '{
    "decision": "approved",
    "user_id": "alice",
...
```bash
curl -s http://localhost:7700/executions/exec_01JM4X8NKWP2 | jq .status
# "completed"
```

To reject instead, pass `--decision rejected` to the CLI (or `"rejected"` in the curl body). The execution status will be `"failed"` with the decider and comment in the failure reason.
