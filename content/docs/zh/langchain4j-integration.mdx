...
使用 JamJet 封装您的 LangChain4j 智能体，无需更改应用程序代码即可获得以下能力：

| 能力 | 不使用 JamJet | 使用 JamJet |
|------------|---------------|-------------|
| **崩溃恢复** | 进程终止，交互丢失，Token 浪费 | 运行时跟踪执行，重启后可恢复 |
| **审计跟踪** | 无操作记录 | 每个方法调用都记录为不可变事件，包含参数、结果和状态 |
| **回放测试** | 必须调用真实 LLM 进行测试 | 在测试套件中回放已记录的执行，无需调用 LLM |
| **成本跟踪** | 手动计算 Token | 执行事件包含方法名和参数，用于成本归因 |
| **可观测性** | 仅应用程序级日志 | 执行 ID 用于分布式跟踪关联，通过 Spring Boot starter 提供 Micrometer 指标 |
| **对话内存持久化** | 仅内存存储，重启后丢失 | 通过 JamJet 审计事件系统持久化，可跨重启存活 |

---

## 测试封装的代理

`jamjet-spring-boot-starter-test` 模块支持封装的 LangChain4j 代理。由于 `JamjetDurableAgent.wrap()` 会创建 JamJet 执行记录,您可以在测试中使用 `@ReplayExecution` 重放它们。
...
有关完整的测试 API ——`RecordedExecution` 字段、`AgentAssertions` 流式 API、`DeterministicModelStub`、节点分叉重放——请参阅 [Spring Boot Starter](/spring-boot-starter) 指南的测试部分。

---

## 下一步

- **[Java SDK 参考](/java-sdk)** — 对于新项目,JamJet 的原生 Java SDK 提供直接的 LLM 集成、类型化工具、策略选择和 IR 编译,无需依赖 LangChain4j
- **[Java 快速入门](/java-quickstart)** — 使用原生 SDK 从零开始构建您的第一个代理和工作流
- **[Spring Boot Starter](/spring-boot-starter)** — 完整的 Spring AI 集成,具备自动配置的持久性、审计跟踪、人工审批和可观测性
- **[代理 AI 模式](https://sunilprakash.com/agentic-ai)** — 代理系统的策略选择、工具设计和生产模式
