...
<Callout type="info">
  The declarative `policy('require_approval', ...)` rule does **not** automatically create approvals. It records intent on the span but does not place a request in the queue. Use `requireApproval()` from code, or the policy-gateway flow below, for actual gating.
</Callout>
