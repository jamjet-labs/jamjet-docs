...
- **[LangChain4j 통합](/langchain4j-integration)** --- `JamjetDurableAgent`와 `JamjetChatMemoryStore`를 사용하여 LangChain4j 에이전트를 위한 지속 가능한 실행 레이어로 JamJet 활용
- **[Java SDK 레퍼런스](/java-sdk)** --- 도구, 전략, IR 컴파일, 런타임 클라이언트에 대한 전체 API 문서
- **[Java 빠른 시작](/java-quickstart)** --- Java SDK로 처음부터 에이전트와 워크플로우 구축하기
- **[핵심 개념](/concepts)** --- 에이전트, 노드, 상태, 지속성에 대한 심층 이해
- **[에이전틱 AI 패턴](https://sunilprakash.com/agentic-ai)** --- 에이전트 시스템을 위한 전략 선택, 도구 설계, 프로덕션 패턴
