...
[Engram](https://java-ai-memory.dev) es la capa de memoria duradera de JamJet para agentes de IA. Va más allá del simple historial de chat: Engram extrae entidades y relaciones de las conversaciones, construye un grafo de conocimiento temporal y soporta recuperación semántica mediante embeddings. Cuando se combina con el starter de Spring Boot, proporciona a tus agentes memoria persistente y consultable a largo plazo que sobrevive reinicios y escala entre sesiones.

### Añadir la dependencia

El starter de Engram es un artefacto separado del starter principal de JamJet. Añádelo junto a tus dependencias existentes:
...
### Iniciar el servidor Engram

Ejecuta el servidor Engram con Docker:

```bash
docker run -p 7680:7680 ghcr.io/jamjet-labs/engram-server:latest
```

O descarga una versión específica:
...
Añade la conexión al servidor Engram en tu `application.yml`:
...
El starter autoconfigura un bean `ChatMemoryRepository` respaldado por el servidor Engram. El `MessageChatMemoryAdvisor` de Spring AI utiliza esto para persistir el historial de conversación de forma duradera: sin estado en memoria que perder al reiniciar.
...
Con esta configuración, cada turno de conversación se almacena a través de Engram. El `ChatMemoryRepository` maneja la lectura y escritura del historial de mensajes de forma transparente: tu código de aplicación funciona con `ChatClient` como siempre.

### Más allá del historial de chat

`ChatMemoryRepository` almacena mensajes de conversación sin procesar. Engram hace mucho más por debajo: extrae entidades y relaciones de cada mensaje, les añade marcas de tiempo y las indexa para búsqueda semántica. Esto significa que tu agente puede recordar hechos de conversaciones pasadas --- no solo reproducir registros de mensajes, sino responder preguntas como "¿qué dijo el usuario sobre su presupuesto la semana pasada?" a través de las herramientas MCP de Engram o su API.

Para el conjunto completo de funcionalidades de Engram --- consultas de grafos de conocimiento, búsqueda semántica, backends SQLite y Postgres, herramientas de servidor MCP --- consulta la [documentación de Engram](https://java-ai-memory.dev).
...
| Spring Boot | 3.4+ |
| Spring AI | 1.0+ |
| JamJet runtime | 0.3.1+ (Docker o binario) |

---

## Próximos pasos

- **[Integración con LangChain4j](/langchain4j-integration)** --- usa JamJet como capa de ejecución duradera para agentes LangChain4j con `JamjetDurableAgent` y `JamjetChatMemoryStore`
- **[Referencia del SDK de Java](/java-sdk)** --- cobertura completa de la API para herramientas, estrategias, compilación IR y el cliente runtime
- **[Inicio rápido con Java](/java-quickstart)** --- construye tu primer agente y flujo de trabajo desde cero con el SDK de Java
