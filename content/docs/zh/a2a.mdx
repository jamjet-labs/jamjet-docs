...
JamJet 实现了 [Agent-to-Agent (A2A) 协议](https://google.github.io/A2A/) —— 这是一个开放标准,允许代理跨框架和组织相互发现、调用和协作。

> **关键区别:** JamJet 将 A2A 协作纳入持久化运行时模型,使代理之间的委托保持可追溯、可重放,并可在运行时限制下强制执行。
...
- [快速开始](/docs/quickstart) —— 先运行一个工作流
- [MCP 集成](/docs/mcp) —— 连接到外部工具服务器
- [企业安全](/docs/enterprise) —— A2A 的 mTLS、OAuth 和联邦控制
