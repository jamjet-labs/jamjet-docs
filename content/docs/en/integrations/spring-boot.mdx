...
[Engram](https://java-ai-memory.dev) is JamJet's durable memory layer for AI agents. It goes beyond simple chat history --- Engram extracts entities and relations from conversations, builds a temporal knowledge graph, and supports semantic recall via embeddings. When paired with the Spring Boot starter, it gives your agents persistent, searchable long-term memory that survives restarts and scales across sessions.

### Add the dependency

The Engram starter is a separate artifact from the core JamJet starter. Add it alongside your existing dependencies:
...
Run the Engram server with Docker:
...
Add the Engram server connection to your `application.yml`:
...
The starter auto-configures a `ChatMemoryRepository` bean backed by the Engram server. Spring AI's `MessageChatMemoryAdvisor` uses this to persist conversation history durably --- no in-memory state to lose on restart.
...
With this configuration, every conversation turn is stored through Engram. The `ChatMemoryRepository` handles reading and writing message history transparently --- your application code works with `ChatClient` as usual.

### Beyond chat history

`ChatMemoryRepository` stores raw conversation messages. Engram does more than that under the hood: it extracts entities and relations from each message, timestamps them, and indexes them for semantic search. This means your agent can recall facts from past conversations --- not just replay message logs, but answer questions like "what did the user say about their budget last week?" through the Engram MCP tools or API.

For the full Engram feature set --- knowledge graph queries, semantic search, SQLite and Postgres backends, MCP server tools --- see the [Engram documentation](https://java-ai-memory.dev).
...
- **[LangChain4j Integration](/langchain4j-integration)** --- use JamJet as a durable execution layer for LangChain4j agents with `JamjetDurableAgent` and `JamjetChatMemoryStore`
