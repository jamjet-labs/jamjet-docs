...
커스텀 실험 파이프라인 구축 없이 재현 가능한 멀티 에이전트 실험을 실행하세요. JamJet은 내구성 있는 실행, 전략 스윕, 재생, 평가, 그리고 논문 수준의 내보내기를 하나의 런타임에서 제공합니다.

이 가이드는 프로젝트 스캐폴드부터 논문 준비 완료 결과까지 완전한 연구 워크플로를 안내합니다.

> **팁:** 
JamJet에 이미 익숙하신가요? [실험 실행](#run-experiments) 또는 [논문용 내보내기](#export-for-publication)로 바로 이동하세요.

> **JamJet 연구 워크플로를 사용해야 할 때:**
> - 모델과 시드 간 추론 전략 비교
> - 자동 결과 수집과 함께 파라미터 스윕 실행
> - 체크포인트에서 실패했거나 흥미로운 조건 재생
> - 논문용 결과 내보내기 (LaTeX, CSV, JSON과 유의성 검정 포함)
> - 나중에 실험을 프로덕션 워크플로로 연결
