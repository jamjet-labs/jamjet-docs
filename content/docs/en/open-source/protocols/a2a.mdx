...
JamJet implements the [Agent-to-Agent (A2A) protocol](https://a2a-protocol.org/) — an open standard for agents to discover, call, and collaborate with each other across frameworks and organizations.

> **Key difference:** JamJet makes A2A collaboration part of the durable runtime model, so delegation between agents remains traceable, replayable, and enforceable under runtime limits.
...
- [Quickstart](/open-source/quickstart) — get a workflow running first
- [MCP Integration](/open-source/protocols/mcp) — connect to external tool servers
- [Enterprise Security](/docs/enterprise) — mTLS, OAuth, and federation controls for A2A
