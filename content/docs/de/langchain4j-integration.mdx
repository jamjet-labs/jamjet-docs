...
### Engram: Langzeitgedächtnis über den Chat-Verlauf hinaus

`JamjetChatMemoryStore` persistiert rohe Konversationsnachrichten über die JamJet-Runtime. Für Agenten, die umfangreicheres Gedächtnis benötigen --- Entitätsextraktion, temporale Wissensgraphen, semantische Suche --- ist [Engram](https://java-ai-memory.dev) die dedizierte Memory-Ebene von JamJet. Der `engram-spring-boot-starter` stellt ein `ChatMemoryRepository` für Spring AI bereit, das Nachrichten über den Engram-Server speichert und zusätzlich Entitäten und Beziehungen extrahiert und für semantischen Abruf indiziert. Details zur Einrichtung finden Sie im Abschnitt [Engram Memory](/spring-boot-starter#engram-memory) im Spring Boot Starter Guide.

---

## Was Sie erhalten

Wenn Sie Ihre LangChain4j-Agenten mit JamJet wrappen, erhalten Sie folgende Funktionen, ohne Ihren Anwendungscode zu ändern:

| Funktion | Ohne JamJet | Mit JamJet |
|------------|---------------|-------------|
| **Crash-Recovery** | Prozess stirbt, Interaktion verloren, Tokens verschwendet | Ausführung von Runtime verfolgt, nach Neustart fortsetzbar |
| **Audit-Trails** | Keine Aufzeichnung der Vorgänge | Jeder Methodenaufruf als unveränderliches Event mit Argumenten, Ergebnis und Status protokolliert |
| **Replay-Testing** | Live-LLM-Aufruf zum Testen erforderlich | Aufgezeichnete Ausführungen in Test-Suite replizierbar, keine LLM-Aufrufe nötig |
| **Kostenverfolgung** | Manuelles Token-Zählen | Execution-Events enthalten Methodenname und Argumente für Kostenzuordnung |
| **Observability** | Nur Logging auf Anwendungsebene | Execution-IDs für Korrelation im Distributed Tracing, Micrometer-Metriken via Spring Boot Starter |
| **Chat-Memory-Persistierung** | Nur im Speicher, nach Neustart verloren | Dauerhaft über JamJet-Audit-Event-System, übersteht Neustarts |

---

## Testen von umschlossenen Agenten

Das Modul `jamjet-spring-boot-starter-test` funktioniert mit umschlossenen LangChain4j-Agenten. Da `JamjetDurableAgent.wrap()` JamJet-Ausführungen erstellt, können Sie diese in Tests mit `@ReplayExecution` wiedergeben.
...
Fügen Sie die Test-Dependency hinzu:
...
Für die vollständige Test-API — `RecordedExecution`-Felder, `AgentAssertions` Fluent-API, `DeterministicModelStub`, Fork-at-Node-Wiedergabe — siehe den Testing-Abschnitt im [Spring Boot Starter](/spring-boot-starter)-Leitfaden.

---

## Nächste Schritte

- **[Java SDK-Referenz](/java-sdk)** — für neue Projekte bietet das native Java SDK von JamJet direkte LLM-Integration, typisierte Tools, Strategieauswahl und IR-Kompilierung ohne die LangChain4j-Abhängigkeit
- **[Java-Schnellstart](/java-quickstart)** — erstellen Sie Ihren ersten Agenten und Workflow von Grund auf mit dem nativen SDK
- **[Spring Boot Starter](/spring-boot-starter)** — vollständige Spring AI-Integration mit automatisch konfigurierter Dauerhaftigkeit, Audit-Trails, Human-in-the-Loop-Freigabe und Observability
