...
- **Runs** (`/dashboard/traces`): filter by `user_id` to see all LLM calls for a specific user. Useful for support tickets and abuse investigations.
- **Costs** (Explorer): group spend by `user_id` to produce per-user cost reports.
- **Graph**: filter by user to isolate the agent chains that served a particular request.
...
<Cards>
  ...
  <Card title="Runs" href="/cloud/telemetry/spans-and-traces" description="Filter runs and spans by user in the dashboard." />
