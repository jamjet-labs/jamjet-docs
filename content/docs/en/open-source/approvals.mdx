...
<Callout type="info">
  `blocked_tools` takes precedence over `require_approval_for`. A tool matched by both is refused outright and the node fails immediately, rather than being parked.
</Callout>
...
| `409` | Nothing is pending, or `node_id` does not match a pending node, or the execution is already in a terminal state (`completed`, `failed`, `cancelled`). |
...
curl -s -X POST http://localhost:8080/executions \
  -H "Content-Type: application/json" \
  -d '{"workflow_id": "payment-processor", "state": {"payment_id": "pay_001"}}'
```
...
A complete runnable approval workflow lives in [`examples/hitl-approval`](https://github.com/jamjet-labs/jamjet/tree/main/examples/hitl-approval) in the main repository.
