...
- **Telemetry**: every LLM call captured as a span, with dashboards, search, and audit
- **Multi-agent attribution**: a network graph of which agent did what
- **Cost optimization**: surface waste findings and turn them into enforced policy in one click
- **Savings**: see the spend that enforcement actually recovered, metered not self-reported
...
- **JamJet Cloud:** Free tier with generous limits. Production usage starts at $0.50 per million spans. See [app.jamjet.dev](https://app.jamjet.dev) for current pricing.
