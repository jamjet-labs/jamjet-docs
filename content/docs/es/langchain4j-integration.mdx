...
### Engram: memoria a largo plazo más allá del historial de chat

`JamjetChatMemoryStore` persiste mensajes de conversación sin procesar a través del runtime de JamJet. Para agentes que necesiten memoria más rica --- extracción de entidades, grafos de conocimiento temporales, búsqueda semántica --- considera [Engram](https://java-ai-memory.dev), la capa de memoria dedicada de JamJet. El `engram-spring-boot-starter` proporciona un `ChatMemoryRepository` para Spring AI que almacena mensajes a través del servidor Engram, el cual además extrae entidades y relaciones y las indexa para recuperación semántica. Consulta la sección [Memoria Engram](/spring-boot-starter#engram-memory) en la guía de Spring Boot Starter para detalles de configuración.

---

## Qué obtienes

Envolver tus agentes LangChain4j con JamJet añade las siguientes capacidades sin cambiar el código de tu aplicación:

| Capacidad | Sin JamJet | Con JamJet |
|------------|---------------|-------------|
| **Recuperación ante fallos** | El proceso muere, interacción perdida, tokens desperdiciados | Ejecución rastreada por el runtime, reanudable tras reinicio |
| **Trazas de auditoría** | Sin registro de lo ocurrido | Cada llamada a método registrada como evento inmutable con argumentos, resultado y estado |
| **Pruebas con replay** | Debe llamar al LLM en vivo para probar | Reproduce ejecuciones grabadas en suite de pruebas, sin llamadas al LLM |
| **Seguimiento de costos** | Conteo manual de tokens | Eventos de ejecución incluyen nombre de método y argumentos para atribución de costos |
| **Observabilidad** | Solo logging a nivel de aplicación | IDs de ejecución para correlación de trazado distribuido, métricas Micrometer via Spring Boot starter |
| **Persistencia de memoria de chat** | Solo en memoria, perdida al reiniciar | Duradera a través del sistema de eventos de auditoría de JamJet, sobrevive reinicios |

---

## Prueba de agentes envueltos
...
Para la API completa de pruebas — campos de `RecordedExecution`, API fluida de `AgentAssertions`, `DeterministicModelStub`, reproducción fork-at-node — consulta la sección de pruebas de la guía [Spring Boot Starter](/spring-boot-starter).

---

## Próximos pasos

- **[Referencia del SDK de Java](/java-sdk)** — para proyectos nuevos, el SDK nativo de Java de JamJet proporciona integración directa con LLM, herramientas tipadas, selección de estrategias y compilación de IR sin la dependencia de LangChain4j
- **[Inicio Rápido de Java](/java-quickstart)** — construye tu primer agente y flujo de trabajo desde cero con el SDK nativo
- **[Spring Boot Starter](/spring-boot-starter)** — integración completa con Spring AI con durabilidad autoconfigurada, registros de auditoría, aprobación humana en el bucle y observabilidad
