...
LangChain4jエージェントをJamJetでラップすると、アプリケーションコードを変更することなく、以下の機能が追加されます:

| 機能 | JamJetなし | JamJetあり |
|------------|---------------|-------------|
| **クラッシュ回復** | プロセスが停止すると、インタラクションが失われ、トークンが無駄になる | ランタイムによって実行が追跡され、再起動後に再開可能 |
| **監査証跡** | 何が起こったかの記録がない | すべてのメソッド呼び出しが引数、結果、ステータスを含む不変のイベントとして記録される |
| **リプレイテスト** | テストするには実際のLLMを呼び出す必要がある | 記録された実行をテストスイートでリプレイでき、LLM呼び出しは不要 |
| **コスト追跡** | 手動でのトークンカウントが必要 | 実行イベントにメソッド名と引数が含まれ、コストの帰属が可能 |
| **可観測性** | アプリケーションレベルのロギングのみ | 分散トレース相関のための実行ID、Spring Boot starterを介したMicrometerメトリクス |
| **チャットメモリの永続化** | メモリ内のみで、再起動時に失われる | JamJet監査イベントシステムによって永続化され、再起動後も存続 |

---

## ラップされたエージェントのテスト

`jamjet-spring-boot-starter-test`モジュールは、ラップされたLangChain4jエージェントと連携します。`JamjetDurableAgent.wrap()`がJamJetの実行を作成するため、`@ReplayExecution`を使用してテストで再生できます。
...
完全なテストAPI（`RecordedExecution`のフィールド、`AgentAssertions`のフルエントAPI、`DeterministicModelStub`、ノード単位での分岐再生）については、[Spring Boot Starter](/spring-boot-starter)ガイドのテストセクションを参照してください。

---

## 次のステップ

- **[Java SDKリファレンス](/java-sdk)** — 新規プロジェクト向けに、JamJetのネイティブJava SDKはLangChain4jの依存関係なしで、直接的なLLM統合、型付きツール、戦略選択、IRコンパイルを提供します
- **[Javaクイックスタート](/java-quickstart)** — ネイティブSDKを使用して、最初のエージェントとワークフローをゼロから構築します
- **[Spring Boot Starter](/spring-boot-starter)** — 自動設定された永続性、監査証跡、ヒューマン・イン・ザ・ループ承認、可観測性を備えた完全なSpring AI統合
- **[エージェンティックAIパターン](https://sunilprakash.com/agentic-ai)** — エージェントシステムのための戦略選択、ツール設計、本番パターン
