...
Hosted vector memory for your agent fleet. Three-level scope (project / agent / end-user), inclusive cascade on recall, BYOK embedding provider. See the [Memory guide](/cloud/memory) for concepts and quickstart.
...
**Request (single):** `{ "id": "abc-...", "reason": "user request" }`
**Request (bulk):** `{ "end_user_id": "u_alpha", "reason": "GDPR delete" }`
